...
pub fn reduce_once(func: &Func) -> Option<Path> {
    const rules: [rewrite::Rule; 9] = [
        rewrite::Rule::ProjCar,
        rewrite::Rule::ProjCdr,
        rewrite::Rule::RecElimZ,
        rewrite::Rule::RecElimS,
        rewrite::Rule::EtaReductionLeft,
        rewrite::Rule::EtaReductionRight,
        rewrite::Rule::CompAssoc,
        rewrite::Rule::CompDistributeEmpty,
        rewrite::Rule::CompDistributeStack,
    ];
    for rule in &rules {
        if let Some(rw) = rule.match_start(func) {
            return Some(Path::validate(View::Rewrite(rw)).unwrap());
        }
    }

   if let FView::Comp(f, g) = func.view() {
       let opt = None.or_else(|| reduce_once(f).map(|rw| Path::validate(View::CompLeft(rw, g.clone())).unwrap()))
       .or_else(|| reduce_once(g).map(|rw| Path::validate(View::CompRight(f.clone(),rw)).unwrap()));
       if let Some(p) = opt {
           return Some(p);
       }
   }
   if let FView::Stack(car, cdr) = func.view() {
       let opt = None.or_else(|| reduce_once(car).map(|rw| Path::validate(View::StackCar(rw, cdr.clone())).unwrap()))
       .or_else(|| reduce_once(cdr).map(|rw| Path::validate(View::StackCdr(car.clone(),rw)).unwrap()));
       if let Some(p) = opt {
           return Some(p);
       }
   }
